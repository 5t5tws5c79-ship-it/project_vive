// 크로스페이드 플레이어 — <audio> 2채널 볼륨 램프. audio 엘리먼트 2개만 주면 됨.
// cur = 현재 트랙 채널(크로스페이드 시작 즉시 갱신) → pause/resume 이 항상 올바른 채널을 잡음.
export function createCrossfadePlayer(el0, el1) {
  const au = [el0, el1];
  ...
  return {
    crossfade,
    // 재생 멈춤: 페이드 중단 + 양쪽 채널 정지 → 실제로 소리 멈춤
    pause() { stopTimer(); try { au[0].pause(); } catch (e) {} try { au[1].pause(); } catch (e) {} },
    // 재개: 현재 채널을 제 볼륨으로 되살리고 반대 채널은 확실히 정지
    resume() {
      try { au[cur].volume = vol/100; const p = au[cur].play(); if (p && p.catch) p.catch(() => {}); } catch (e) {}
      try { au[1-cur].pause(); } catch (e) {}
    },
    setVolume(v) { vol = v; try { au[cur].volume = v/100; } catch (e) {} },
    stop() { stopTimer(); try { au[0].pause(); au[1].pause(); au[0].currentTime = 0; au[1].currentTime = 0; } catch (e) {} },
  };
}
