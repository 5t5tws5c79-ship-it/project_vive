...
import { MOODS, TYPE_TO_MOOD, TYPE_LABEL } from './moodConfig.js';
...
export async function llmMood(list, hour, key) {
  ...
  const body = {
    ...
    response_format: { type:"json_schema", json_schema:{ name:"mood", strict:true, schema:{
...
export async function inferMood(list, hour, key) {
  if (key) {
    ...
    try {
      ...
      const r = ruleMood(list, hour); r.note = "LLM 실패(" + e.message + ")→규칙"; return r;
    }
  }
