...
프로젝트 초기에는 카카오맵 API를 단순히 화면에 표시하는 것부터 시작했지만, 실제 서비스에 적용하기 위해서는 API 키 설정, 도메인 등록, 비동기 데이터 처리 등 여러 요소를 고려해야 했습니다. 이를 통해 단순히 기능을 구현하는 것을 넘어, 외부 서비스를 프로젝트 구조에 맞게 설계하고 안정적으로 연결하는 경험을 얻었습니다.
또한, API Key를 코드에 직접 작성하여 GitHub에 노출될 수 있는 문제가 있었고 외부 API Key와 같은 민감한 정보는 반드시 환경 변수로 관리해야 한다는 점을 잊지 말아야 합니다.

- 임상준

날씨 데이터를 활용하여 무드 추측 정확도를 높이는 방안도 추후 개선점이 될 것 같습니다. 현 위치 기반의 음악 공유에 더하여 움직인 동선 기반의 추천 음악 공유 기능도 유용하다 생각됩니다.
시간 분배 등의 현실적 이유로 인해 필요한 기능에 선택과 집중을 해야한다. 기능 우선순위 선정이 중요하다. 다회차의 경험을 통해 얼마나 시간/분량이 투입될지 예측하는 감도 필요하다.
